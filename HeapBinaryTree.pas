...
type
	...
	root: THeapBinaryTreeNode;
        constructor create();
        procedure addTreeNode(rank, x, y, xa, ya: integer);
        procedure printTree();
...
procedure THeapBinaryTree.addTreeNode(rank, x, y, xa, ya: integer);
...
begin
    ...
    newTreeNode.setRank(Rank);
    newTreeNode.setX(x);
    newTreeNode.setY(y);
    newTreeNode.setXa(xa);
    newTreeNode.setYa(ya);
...
begin
    ...
    parent := root;
    isLeftChild := true;
    while current.getRank <> key do
        begin
            writeln(current.getRank);
            parent := current;
            if key < current.getRank then
                begin
                    isLeftChild := true;
                    current := current.getLeftChild
                end
